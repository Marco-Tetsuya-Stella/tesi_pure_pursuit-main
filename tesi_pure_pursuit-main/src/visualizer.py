...
# Trova la riga degli import da typing e aggiungi Callable:
...
# ------------------------------------------------------------------------------
# NUOVE FUNZIONI AGGIUNTE PER LA TESI RIGUARDANTE IL PURE PURSUIT
# ------------------------------------------------------------------------------
#funzione che ha il compito di visualizzare solo il percorso da seguire usando Open 3d

def show_path_only_carousel(
        histories,
        titles,
        show_orient_every=None,
        _commands_list=None,
        _dts=0.05,
        _show_info=True,
        environment=None,
    ...
    """
    Versione modificata del Carousel che mostra UN SOLO pannello (senza ICP).
    Mantiene l'interfaccia interattiva (Play, Pausa, Step) e il movimento del robot.
    """
    import matplotlib.widgets as pywidgets
    ...
    for h in histories:
        ...
        if h_np.ndim == 2 and h_np.shape[1] == 3:
            histories_clean.append(h_np)
    if not histories_clean:
        print("Nessuna traiettoria valida da visualizzare.")
        return

    num_cases = len(histories_clean)
    current_case = [0]  # usa lista per mutabilità nelle closure
    current_frame = [0]
    is_playing = [False]
    timer_obj = [None]

    # Configurazione della Figura (Unico pannello capiente)
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(1, 1, 1)

    # Riferimenti per gli elementi grafici da aggiornare
    drawn_elements = []

    def draw_frame():
        # Rimuove i vecchi disegni del robot
        while drawn_elements:
            try:
                drawn_elements.pop().remove()
            except Exception:
                pass

        hist_np = histories_clean[current_case[0]]
        title = titles[current_case[0]] if current_case[0] < len(titles) else "Traiettoria"

        ax.clear()
        ax.set_title(f"Visualizzazione Percorso: {title}", fontsize=12, fontweight='bold')

        # 1. Disegna l'ambiente (se fornito)
        if environment is not None:
            curr_env = environment[current_case[0]] if isinstance(environment, list) else environment
            if curr_env is not None:
                curr_env.plot(ax)

        # 2. Disegna la linea dell'intero percorso (statica)
        ax.plot(hist_np[:, 0], hist_np[:, 1], '-', color='black', alpha=1, label='Percorso di riferimento')

        # 3. Disegna i punti di orientamento (frecce statiche) se richiesto
        #step_orient = 10
        #if show_orient_every is not None:
            #step_orient = show_orient_every[current_case[0]] if isinstance(show_orient_every,
                                                                           #list) else show_orient_every

        #if step_orient and step_orient > 0:
            #for idx in range(0, len(hist_np), step_orient):
                #pose = hist_np[idx]
                #q = ax.quiver(pose[0], pose[1], np.cos(pose[2]), np.sin(pose[2]),
                              #color='darkblue', alpha=0.4, scale=25, width=0.003)
                #drawn_elements.append(q)

        # 4. Disegna il robot nella sua posizione corrente (dinamica)
        f = min(current_frame[0], len(hist_np) - 1)
        curr_state = hist_np[f]

        # Utilizza la funzione draw_robot nativa del tuo visualizer
        r_patches = draw_robot(ax, curr_state, robot_radius=0.1, color='tab:blue')
        drawn_elements.extend(r_patches)

        # 5. Imposta i limiti della mappa
        if _fit_to == 'environment' and environment is not None:
            # Si adatta all'ambiente
            curr_env = environment[current_case[0]] if isinstance(environment, list) else environment
            if curr_env and curr_env.bounds:
                x0, y0, x1, y1 = curr_env.bounds.bounds
                ax.set_xlim(x0, x1)
                ax.set_ylim(y0, y1)
        else:
            # Si adatta alla traiettoria
            margin = 1.0
            ax.set_xlim(np.min(hist_np[:, 0]) - margin, np.max(hist_np[:, 0]) + margin)
            ax.set_ylim(np.min(hist_np[:, 1]) - margin, np.max(hist_np[:, 1]) + margin)

        ax.set_aspect('equal', 'box')
        ax.grid(True, linestyle=':', alpha=0.6)
        fig.canvas.draw_idle()

    # Logica di controllo del Timer per l'animazione
    def on_timer():
        hist_np = histories_clean[current_case[0]]
        if current_frame[0] < len(hist_np) - 1:
            current_frame[0] += 1
            draw_frame()
        else:
            is_playing[0] = False
    ...
    timer_obj[0] = fig.canvas.new_timer(interval=int(_dts * 1000))
    timer_obj[0].add_callback(on_timer)

    # Definizione dei pulsanti interattivi in basso
    plt.subplots_adjust(bottom=0.2)
    ax_prev_case = plt.axes([0.15, 0.05, 0.1, 0.06])
    ax_play = plt.axes([0.3, 0.05, 0.1, 0.06])
    ax_stop = plt.axes([0.45, 0.05, 0.1, 0.06])
    ax_step = plt.axes([0.6, 0.05, 0.1, 0.06])
    ax_next_case = plt.axes([0.75, 0.05, 0.1, 0.06])

    btn_prev_case = pywidgets.Button(ax_prev_case, '◀ Percorso')
    btn_play = pywidgets.Button(ax_play, '▶ Play')
    btn_stop = pywidgets.Button(ax_stop, '■ Pause')
    btn_step = pywidgets.Button(ax_step, 'Step ▶')
    btn_next_case = pywidgets.Button(ax_next_case, 'Percorso ▶')
    ...
    def step_cb(event):
        stop_cb(None)
        hist_np = histories_clean[current_case[0]]
        current_frame[0] = (current_frame[0] + 1) % len(hist_np)
        draw_frame()

    def change_case(offset):
        stop_cb(None)
        current_case[0] = (current_case[0] + offset) % num_cases
        current_frame[0] = 0
        draw_frame()

    btn_play.on_clicked(play_cb)
    btn_stop.on_clicked(stop_cb)
    btn_step.on_clicked(step_cb)
    btn_prev_case.on_clicked(lambda e: change_case(-1))
    btn_next_case.on_clicked(lambda e: change_case(1))
