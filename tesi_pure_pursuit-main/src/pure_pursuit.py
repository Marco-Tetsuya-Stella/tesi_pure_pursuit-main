...
class PurePursuitController:
    ...
    def __init__(
        ...
        """
        Inizializza i parametri operativi del controllore Pure Pursuit[cite: 13].

        Args:
            lookahead_distance: Raggio (m) entro cui il robot cerca il punto di mira sul percorso[cite: 13].
            max_angular_velocity: Limite massimo per la velocità di rotazione del robot[cite: 13].
            target_linear_velocity: Velocità di avanzamento costante desiderata per il robot[cite: 13].
            stop_tolerance: Distanza euclidea finale (m) sotto la quale il traguardo è considerato raggiunto[cite: 13].
            max_index_gap: Numero di waypoint massimi tollerati tra l'indice attuale e la fine del percorso
                           per confermare l'arrivo al traguardo[cite: 13].
        """
        self.L_d = lookahead_distance
        self.max_omega = max_angular_velocity
        self.target_v = target_linear_velocity

        # Indice di progresso lungo il path: serve a non cercare punti del percorso già superati[cite: 13]
        self._last_idx = 0
    ...
    def reset(self) -> None:
        """
        Resetta il progresso lungo il path (da chiamare se si cambia percorso)[cite: 13].

        Returns:
            Nessun valore restituito, azzera semplicemente l'indice interno[cite: 13].
        """
        self._last_idx = 0
    ...
    def find_lookahead_point(
        ...
        circonferenza di raggio L_d centrata sul robot e ciascun segmento del path[cite: 13].

        Args:
            robot_pose: Array [x, y, theta] con la posa globale attuale del robot[cite: 13].
            path: Array Nx2 o Nx3 contenente i waypoint della traiettoria da seguire[cite: 13].

        Returns:
            np.ndarray: Coordinate 2D [x, y] del punto di lookahead ottimale trovato[cite: 13].
        """
        # Estrae le coordinate del robot[cite: 13]
        x_r, y_r, _ = robot_pose
        current_pos = np.array([x_r, y_r], dtype=float)
        n = len(path)
        L_d = self.L_d

        # Se il percorso ha meno di 2 punti, restituisce semplicemente l'ultimo punto disponibile[cite: 13]
        if n < 2:
            return path[-1, :2]

        goal_pt = None
        # Inizia a cercare partendo dall'ultimo indice visitato, per non guardare indietro[cite: 13]
        start_index = min(self._last_idx, n - 2)

        # Scorre i segmenti del percorso[cite: 13]
        for i in range(start_index, n - 1):
            # Trasla il segmento di percorso ponendo il robot all'origine (0,0) per facilitare i calcoli[cite: 13]
            ...
            # Calcola il quadrato della lunghezza del segmento di percorso corrente[cite: 13]
            dr2 = dx * dx + dy * dy

            # Se i due punti del segmento coincidono (distanza zero), salta al prossimo[cite: 13]
            if dr2 < 1e-12:
                continue

            # Applica la formula matematica dell'intersezione retta-circonferenza[cite: 13]
            D = x1 * y2 - x2 * y1
            discriminant = (L_d ** 2) * dr2 - D ** 2

            # Se il discriminante è negativo, il cerchio di raggio L_d non interseca questo segmento[cite: 13]
            if discriminant < 0:
                continue

            # Se c'è intersezione, calcola le possibili soluzioni (fino a due punti di intersezione)[cite: 13]
            sqrt_disc = np.sqrt(discriminant)
            sgn_dy = 1.0 if dy >= 0 else -1.0

            # Soluzioni per la prima intersezione[cite: 13]
            sol_x1 = (D * dy + sgn_dy * dx * sqrt_disc) / dr2
            sol_y1 = (-D * dx + abs(dy) * sqrt_disc) / dr2

            # Soluzioni per la seconda intersezione[cite: 13]
            sol_x2 = (D * dy - sgn_dy * dx * sqrt_disc) / dr2
            sol_y2 = (-D * dx - abs(dy) * sqrt_disc) / dr2

            # Ri-trasla le soluzioni nelle coordinate globali originali[cite: 13]
            sol_pt1 = np.array([sol_x1, sol_y1]) + current_pos
            sol_pt2 = np.array([sol_x2, sol_y2]) + current_pos

            # Determina i confini del segmento di retta originario[cite: 13]
            ...
            # Verifica che il punto di intersezione cada effettivamente DENTRO i limiti del segmento[cite: 13]
            ...
            # Se nessuna intersezione è sul segmento, passa oltre[cite: 13]
            if not (pt1_valid or pt2_valid):
                continue

            # Se entrambe le intersezioni sono valide, sceglie quella più vicina al punto finale del segmento (per procedere in avanti)[cite: 13]
            next_pt = path[i + 1, :2]
            ...
            # Assicura che il punto scelto sia effettivamente più avanti rispetto alla posizione attuale[cite: 13]
            if np.linalg.norm(candidate - next_pt) <= np.linalg.norm(current_pos - next_pt):
                goal_pt = candidate
                # Aggiorna l'indice così al prossimo ciclo partirà da qui[cite: 13]
                self._last_idx = i
                break
            else:
                self._last_idx = i + 1

        # Fallback: se nessuna intersezione viene trovata, punta semplicemente all'ultimo punto visitato valido[cite: 13]
        if goal_pt is None:
    ...
    def compute_commands(
        ...
        """
        Calcola i comandi cinematici per il robot al fine di raggiungere il lookahead point[cite: 13].

        Args:
            robot_pose: Posizione e orientamento attuali del robot [x, y, theta][cite: 13].
            path: L'intera matrice del percorso da seguire[cite: 13].

        Returns:
            Una tupla (v, omega) dove v è la velocità lineare e omega la velocità angolare[cite: 13].
        """
        x_r, y_r, theta_r = robot_pose

        # Controllo di arrivo a destinazione basato sugli indici [cite: 13]
        # Recupera l'ultimo punto del tracciato per usarlo come traguardo[cite: 13]
        final_point = path[-1, :2]
        # Calcola la distanza fisica euclidea tra il robot e il traguardo[cite: 13]
        distance_to_goal = np.hypot(final_point[0] - x_r, final_point[1] - y_r)

        # Calcoliamo quanti punti "logici" mancano alla fine del tracciato rispetto al nostro progresso[cite: 13]
        index_distance_to_end = (len(path) - 1) - self._last_idx

        # Se la distanza fisica è inferiore alla soglia E siamo vicini alla fine dell'array (es. ultimi 30 punti)[cite: 13]
        # OPPURE se l'indice è rimasto bloccato vicinissimo alla fine (ultimi 3 punti dell'array)[cite: 13]
        if distance_to_goal < self.stop_tolerance:
            if index_distance_to_end < self.max_index_gap:
                # Il robot si ferma inviando velocità zero[cite: 13]
                return 0.0, 0.0
        # --------------------------------------------------------------------------[cite: 13]

        # Richiama la funzione geometrica per trovare il punto di mira[cite: 13]
        ...
        # Calcola la differenza (delta) globale tra il punto di mira e il robot[cite: 13]
        dx = lookahead_pt[0] - x_r
        dy = lookahead_pt[1] - y_r

        # Trasforma i delta globali (dx, dy) nel sistema di coordinate locale del robot tramite matrice di rotazione inversa[cite: 13]
        ...
        # il robot ruota sul posto per rimettersi in direzione[cite: 13]
        if local_x < 0:
            v = self.target_v
            omega = np.sign(local_y) * self.max_omega
            return v, omega

        # Calcola la distanza esatta del lookahead nel sistema locale per prevenire divisioni per zero[cite: 13]
        ...
        # Formula centrale del Pure Pursuit: calcola la curvatura dell'arco che unisce il robot al punto[cite: 13]
        curvature = (2.0 * local_y) / (actual_L_d ** 2)

        # La velocità lineare è mantenuta costante, mentre quella angolare si calcola moltiplicando v per la curvatura[cite: 13]
        v = self.target_v
        # La velocità angolare viene tagliata ("clippata") affinché non superi il limite fisico imposto[cite: 13]
